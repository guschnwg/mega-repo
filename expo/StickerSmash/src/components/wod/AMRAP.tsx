...
interface WodAmrapProps {
  step: AMRAPStepType;
  onEnd: (config: AMRAPConfigType) => void;
  onStop: () => void;
}

export const WodAmrap = ({ step, onEnd, onStop }: WodAmrapProps) => {
    ...
    >
      <Timer
        ...
        onChange={(next) => {
          ...
          if (actualCurrentMs > step.config.time && !ref.current) {
            ref.current = true;
            setTimeout(() => {
              onEnd({
                ...step.config,
